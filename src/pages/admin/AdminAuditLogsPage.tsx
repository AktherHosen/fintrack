...
export function AdminAuditLogsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-black text-zinc-900 dark:text-zinc-50 tracking-tight">Security & Audit Logs</h2>
        <p className="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400">Chronological trail of auth events, transactions, and admin approvals</p>
      </div>
      ...
      <Card className="bg-white dark:bg-zinc-900/90 border-zinc-200 dark:border-zinc-800 shadow-xs">
        <CardContent className="p-0">
          <div className="divide-y divide-zinc-200/80 dark:divide-zinc-800">
            {auditLogs.map((log) => (
              <div key={log.id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-zinc-50/70 dark:hover:bg-zinc-900/40 transition-colors">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-zinc-900 dark:text-zinc-100 text-sm">{log.action}</span>
