...
export function AdminUsersPage() {
  ...
  const filtered = users
    .filter((u) => {
      ...
      return (
        u.email.toLowerCase().includes(q) || (u.full_name && u.full_name.toLowerCase().includes(q))
      );
    })
    .sort((a, b) => {
    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div>
        ...
        <p className="text-[10px] sm:text-xs text-zinc-500 dark:text-zinc-400 mt-0.5 truncate">
          {customers.length} Customers • {admins.length} System Controller • Manage tier upgrades &
          timelines
        </p>
      </div>
              ...
              <TableBody>
                {filtered.length > 0 ? (
                  filtered.map((u) => {
                    const sub = getUserSub(u.id);
                    const plan = sub?.plan || plans.find((p) => p.id === sub?.plan_id) || plans[0];
                    const { count: orderCount, total: totalSpent } = getUserPaymentsCount(
                      u.id,
                      u.email
                    );
                    const isProPlan = plan && plan.slug !== 'free';
      ...
      <ConfirmDialog
        ...
        description={
          <span>
            Are you sure you want to permanently delete this customer? This will remove all their
            data including{' '}
            <strong>
              accounts, transactions, budgets, loans, subscriptions, and payment history
            </strong>
