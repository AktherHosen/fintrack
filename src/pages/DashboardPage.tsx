      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="hover:border-zinc-700 transition-colors">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <span className="text-xs font-medium text-zinc-400">Total Net Worth</span>
                ...
                <div className="w-full space-y-1.5 mt-2 max-h-28 overflow-y-auto pr-1">
                  {pieData.slice(0, 4).map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-semibold text-zinc-100">{formatCurrency(item.value, currency, locale)}</span>
                    </div>
                  ))}
                </div>
              </>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1">
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            ...
            <Link to="/accounts" className="text-xs text-zinc-400 hover:text-zinc-100 hover:underline flex items-center gap-0.5">
              <span>View all</span>
            ...
            {accounts.map((acc) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <div className="h-8 w-8 rounded-md bg-zinc-800 text-zinc-300 flex items-center justify-center font-bold text-xs">
                  ...
                  <div>
                    <h4 className="text-xs font-semibold text-zinc-100 leading-tight">{acc.name}</h4>
                    <span className="text-[10px] text-zinc-500 font-mono">{acc.account_number || acc.type}</span>
                  </div>
                </div>
                <div className={`text-xs font-semibold ${Number(acc.balance) < 0 ? 'text-rose-400' : 'text-zinc-100'}`}>
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            ...
            <Link to="/transactions" className="text-xs text-zinc-400 hover:text-zinc-100 hover:underline flex items-center gap-0.5">
              ...
              <TableBody>
                {transactions.slice(0, 5).map((tx) => (
                  <TableRow key={tx.id}>
                    ...
                    <TableCell>
                      {tx.category ? (
                        <Badge variant="outline" className="text-[10px] py-0 h-4 border-zinc-800 text-zinc-400">
                          {tx.category.name}
                        </Badge>
                      ) : (
