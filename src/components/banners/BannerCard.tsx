...
export function BannerCard({
  ...
  return (
    <div
      className={cn(
        'relative overflow-hidden rounded-lg border shadow-xs transition-all duration-300 h-16',
        hasImage
          ? 'text-white'
          : hasCustomBg
            ...
            >
              <svg className="w-full h-full -rotate-90 transform" viewBox="0 0 24 24">
                ...
                <circle
                  ...
                  strokeLinecap="round"
                  className="fill-none text-white transition-[stroke-dashoffset] duration-75 ease-linear"
                />
              </svg>
            </div>
            <Badge
              variant="outline"
              className="text-[7px] py-0 h-3 px-1 font-black tracking-widest uppercase rounded select-none leading-none shadow-none bg-white/20 border-white/30 text-white"
            >
              AD
            </Badge>
          </div>
          ...
          <div className="absolute inset-0 p-2.5 sm:p-3 flex flex-col justify-end">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0 flex-1">
                {banner.title && (
                  <h4 className="text-xs sm:text-[13px] font-bold tracking-tight truncate text-white drop-shadow-sm">
                    {banner.title}
                  </h4>
                )}
                {banner.description && (
                  <p className="text-[10px] sm:text-[11px] truncate text-white/80 leading-tight">
                    {banner.description}
                  </p>
                )}
              </div>
              {banner.button_text && (
                <Button
                  ...
                  className="h-6 sm:h-6.5 text-[10px] sm:text-[11px] font-bold px-2.5 sm:px-3 shadow-xs shrink-0 bg-white text-zinc-950 hover:bg-zinc-100 transition-all hover:scale-[1.02] active:scale-[0.98]"
                >
